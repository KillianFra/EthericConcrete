...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("read_blueprint_function"))
    {
        return HandleReadBlueprintFunction(Params);
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleReadBlueprintFunction(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    // Resolve blueprint — support full /Game/ paths as well as short names
    UBlueprint* Blueprint = nullptr;
    if (BlueprintName.StartsWith(TEXT("/Game/")) || BlueprintName.StartsWith(TEXT("/Script/")))
    {
    ...
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(
            FString::Printf(TEXT("Function '%s' not found in blueprint '%s'"), *FunctionName, *BlueprintName));
    }

    // Serialize all nodes (same format as event graph nodes in read_blueprint)
    TArray<TSharedPtr<FJsonValue>> NodesArray;
    ...
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ResultObj->SetStringField(TEXT("blueprint"), Blueprint->GetName());
    ResultObj->SetStringField(TEXT("function"), FunctionName);
    ResultObj->SetNumberField(TEXT("node_count"), FuncGraph->Nodes.Num());
