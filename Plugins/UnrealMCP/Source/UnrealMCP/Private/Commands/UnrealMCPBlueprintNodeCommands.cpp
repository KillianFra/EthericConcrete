...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("connect_blueprint_function_nodes"))
    {
        return HandleConnectBlueprintFunctionNodes(Params);
    }
    else if (CommandType == TEXT("add_node_to_function_graph"))
    {
        return HandleAddNodeToFunctionGraph(Params);
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleConnectBlueprintFunctionNodes(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'blueprint_name' parameter"));
    ...
    if (!FuncGraph)
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Function graph '%s' not found"), *FunctionName));
    ...
    for (UEdGraphNode* Node : FuncGraph->Nodes)
    {
        ...
        if (GuidStr == SourceNodeId) SourceNode = Node;
        else if (GuidStr == TargetNodeId) TargetNode = Node;
    }

    if (!SourceNode || !TargetNode)
        return FUnrealMCPCommonUtils::CreateErrorResponse(
            FString::Printf(TEXT("Node not found — source: %s, target: %s"),
                SourceNode ? TEXT("ok") : TEXT("missing"),
                TargetNode ? TEXT("ok") : TEXT("missing")));

    if (!FUnrealMCPCommonUtils::ConnectGraphNodes(FuncGraph, SourceNode, SourcePinName, TargetNode, TargetPinName))
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to connect nodes"));
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddNodeToFunctionGraph(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    if (!Params->TryGetStringField(TEXT("graph_name"), GraphName))
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'graph_name'"));

    FString NodeType;
    if (!Params->TryGetStringField(TEXT("node_type"), NodeType))
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'node_type'"));
    ...
    if (!FuncGraph)
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Function graph '%s' not found"), *GraphName));
    ...
    if (NodeType == TEXT("VariableGet"))
    {
        FString VariableName;
        if (!Params->TryGetStringField(TEXT("variable_name"), VariableName))
            return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'variable_name' for VariableGet"));

        UK2Node_VariableGet* GetNode = NewObject<UK2Node_VariableGet>(FuncGraph);
        if (!GetNode)
            return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to create VariableGet node"));

        GetNode->VariableReference.SetSelfMember(FName(*VariableName));
        GetNode->NodePosX = NodePosition.X;
        GetNode->NodePosY = NodePosition.Y;
        FuncGraph->AddNode(GetNode);
    ...
    else if (NodeType == TEXT("FunctionCall"))
    {
        FString FunctionName;
        if (!Params->TryGetStringField(TEXT("function_name"), FunctionName))
            return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'function_name' for FunctionCall"));
        ...
        if (Target.IsEmpty() || Target == TEXT("self"))
        {
            ...
            if (!Function && Blueprint->ParentClass)
                Function = Blueprint->ParentClass->FindFunctionByName(FName(*FunctionName));
        }
        else
        {
            // Try to find target class with various prefixes
            TArray<FString> CandidateNames = { Target, FString(TEXT("A")) + Target, FString(TEXT("U")) + Target };
            for (const FString& Candidate : CandidateNames)
            {
                UClass* TargetClass = FindFirstObject<UClass>(*Candidate, EFindFirstObjectOptions::NativeFirst);
                if (TargetClass)
                {
        ...
        if (!Function)
            return FUnrealMCPCommonUtils::CreateErrorResponse(
                FString::Printf(TEXT("Function '%s' not found on target '%s'"), *FunctionName, *Target));

        UK2Node_CallFunction* CallNode = NewObject<UK2Node_CallFunction>(FuncGraph);
        if (!CallNode)
            return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to create FunctionCall node"));

        CallNode->SetFromFunction(Function);
        CallNode->NodePosX = NodePosition.X;
        CallNode->NodePosY = NodePosition.Y;
        FuncGraph->AddNode(CallNode);
    ...
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(
            FString::Printf(TEXT("Unknown node_type '%s'. Use 'VariableGet' or 'FunctionCall'"), *NodeType));
    }
